...
def predicting_for_scikit_learn(dataset, dataset_path, model_path, output_path, output_path2, batch_size):
    data = {
        "census": census_predict_data,
        "credit": credit_predict_data,
        "bank": bank_predict_data,
    ...
    for batch in range(nb_batches):
        ...
        cur_batch_size = end - start
        X_cur[:cur_batch_size] = X[start:end]
        pros = model.predict_proba(X_cur)
        for i in range(start, end):
            pros_all[i] = pros[i - start]
    ...
    with open(output_path, "wb+") as handle:
        pickle.dump(pros_all, handle)
    with open(output_path2, "wb+") as handle:
        pickle.dump(labels, handle)

def main_for_scikit_learn():
    parser = argparse.ArgumentParser(description="Predicting scores and labels of latent vector")
    parser.add_argument("--dataset", choices=["census", "credit", "bank", "compas", "lsac"],
    ...
    parser.add_argument("--batch_size", type=int, default=64,
                        help="Size of batches for predicting")
    parser.add_argument("--log_label", type=str, default="result")

    args = parser.parse_args()

    loggerT = TimeLogger(file="../main_log.txt", label=args.log_label+",predicting_labels")
    loggerT.start_point()

    predicting_for_scikit_learn(dataset=args.dataset, dataset_path=args.dataset_path,
                                model_path=args.model_path, batch_size=args.batch_size,
                                output_path=args.output_path, output_path2=args.output_path2)

    loggerT.end_point()


if __name__ == "__main__":
    main_for_scikit_learn()
    """
