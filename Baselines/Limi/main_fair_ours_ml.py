...
from utils.config import census, credit, bank, meps, compas, lsac
...
def _parse_args():
    ...
    parser.add_argument('--disc_file_path', type=str, default='disc.csv')
    parser.add_argument('--test_file_path', type=str, default='test.csv')
    parser.add_argument("--log_label", type=str, default="result")

    parser.add_argument("--random_seed", type=int, default=1234)
    parser.add_argument("--step", type=float, default=0.3)
    parser.add_argument("--runtime", type=float, default=3600)
...
def numpy2log(arr, output_path):
    # Disc/test sets are saved as binary .npy (was line-joined CSV): one
    # un-formatted write, read back by Experiments.common.metrics. The row count is
    # unchanged, so the runner's pair counting (rows // 2) still matches.
    np.save(output_path, np.asarray(list(arr), dtype=np.int64))
    return
...
class FairTest:
    ...
    def check_for_error_condition(self, t):
        ...
        for val in range(
            ...
            if val != t[sens - 1]:
                ...
                if label_new != label:
                    self.n_value = val
                    return True, [t, label, tnew, label_new]
        return False, None
    ...
    def evaluate_global(self, inp):
        ...
        if tuple(temp) not in tot_inputs:
            ...
            result, t_tnew = self.check_for_error_condition(inp)
        ...
        if result and (tuple(temp) not in global_disc_inputs):
            ...
            real_result = True
            disc_pair.append(t_tnew[0].tolist() + t_tnew[1].tolist())
            disc_pair.append(t_tnew[2].tolist() + t_tnew[3].tolist())
        return real_result
    ...
    def global_phase_search(self):
        ...
        for ind in tqdm(range(0, len(candidate_dict_data[0]))):
            ...
            use_time = _end - self.start_cpu_time
...
if __name__ == '__main__':
    ...
    loggerT = TimeLogger(file="limi_log.txt", label=opt['log_label'] + ",main_testing")
    loggerT.start_point()

    fairtest = FairTest(opt, logger)
    fairtest.run()

    loggerT.end_point(need_log_time=False)

    used_cpu_time_for_testing = time.process_time() - fairtest.start_cpu_time
    used_real_time_for_testing = time.time() - fairtest.start_time
    # report
    no_disc = len(global_disc_inputs_list)
    no_test = len(tot_inputs)
    prec = no_disc / float(no_test)
    res_log = f"{used_cpu_time_for_testing},{used_real_time_for_testing},{no_disc},{no_test},{prec}"
    print(res_log)
    numpy2log(disc_pair, opt['disc_file_path'])
    numpy2log(tot_inputs, opt['test_file_path'])

    loggerT.log_time(add_res=res_log)
