...
def _parse_args():
    ...
    parser.add_argument("--output_file", type=str, default="_.npy")
    parser.add_argument("--output_file2", type=str, default="_svm.npy")
...
def train_equal():
    ...
    score_train = np.concatenate(
        [
            np.ones(len(positive_train), dtype=int),
            np.zeros(len(negative_train), dtype=int),
        ],
        axis=0,
    )
...
if __name__ == "__main__":
    ...
    loggerT = TimeLogger(file="main_log.txt", label=opt["log_label"] + ",train_latent_boundary")
    loggerT.start_point()
